...
STAGE1_DATA="$GATE_ROOT/TrainDatasetAutoAllRefined"
STAGE2_DATA="$GATE_ROOT/TrainDatasetAutoAllRefinedRehearsal"
...
EXP_NAME="stage3_v3_edge_training_e1_no_quality_gate_v1"
...
$PYTHON "$ROOT/stage3_build_all_refined_dataset.py" \
  ...
  --overwrite \
  > "$ROOT/${EXP_NAME}_build_all_refined_stage1_out.log" \
  2> "$ROOT/${EXP_NAME}_build_all_refined_stage1_err.log"

$PYTHON "$ROOT/stage3_build_mixed_rehearsal_dataset.py" \
  --base_manifest "$STAGE1_DATA/train_manifest.csv" \
  --refined_manifest "$STAGE1_DATA/train_manifest.csv" \
