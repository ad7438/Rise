...
EXP_NAME="stage3_v3_edge_h2_all_refined_stage1_only_v1"
REFINE_ROOT="$ROOT/Dataset/Stage3MaskRefine_AGSP_SVPM_SVAC_v1"
DATA_ROOT="$ROOT/Dataset/Stage3AutoGate_H2_all_refined_v1"
...
STAGE1_SAVE="$SINET_ROOT/snapshot/RISE_${EXP_NAME}_stage1_all_refined"
...
if [[ "${SKIP_BUILD:-0}" != "1" || ! -f "$STAGE1_DATA/train_manifest.csv" ]]; then
  "$PYTHON" "$ROOT/stage3_build_all_refined_dataset.py" \
    --results_jsonl "$REFINE_ROOT/results.jsonl" \
    --dev_manifest_jsonl "$DEV_ROOT/dev_manifest.jsonl" \
    --output_root "$STAGE1_DATA" \
    --overwrite \
    > "$ROOT/${EXP_NAME}_build_all_refined_out.log" \
    2> "$ROOT/${EXP_NAME}_build_all_refined_err.log"
else
  echo "SKIP_BUILD=1 and $STAGE1_DATA/train_manifest.csv exists; reuse H2 all-refined dataset." \
    > "$ROOT/${EXP_NAME}_build_all_refined_out.log"
  : > "$ROOT/${EXP_NAME}_build_all_refined_err.log"
fi
