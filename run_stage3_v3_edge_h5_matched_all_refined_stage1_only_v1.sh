...
EXP_NAME="stage3_v3_edge_h5_matched_all_refined_stage1_only_v1"
DATA_ROOT="$ROOT/Dataset/Stage3AutoGate_H5_matched_all_refined_v1"
...
DECISIONS_CSV="$ROOT/Dataset/Stage3AutoGate_AGSP_SVPM_SVAC_v1/auto_gate_decisions.csv"
...
STAGE1_SAVE="$SINET_ROOT/snapshot/RISE_${EXP_NAME}_stage1_matched_all_refined"
...
if [[ "${SKIP_BUILD:-0}" != "1" || ! -f "$STAGE1_DATA/train_manifest.csv" ]]; then
  "$PYTHON" "$ROOT/stage3_build_matched_all_refined_dataset.py" \
    --reference_manifest "$REF_MANIFEST" \
    --decisions_csv "$DECISIONS_CSV" \
    --output_root "$STAGE1_DATA" \
    --overwrite \
    > "$ROOT/${EXP_NAME}_build_matched_all_refined_out.log" \
    2> "$ROOT/${EXP_NAME}_build_matched_all_refined_err.log"
else
  echo "SKIP_BUILD=1 and $STAGE1_DATA/train_manifest.csv exists; reuse H5 matched all-refined dataset." \
    > "$ROOT/${EXP_NAME}_build_matched_all_refined_out.log"
  : > "$ROOT/${EXP_NAME}_build_matched_all_refined_err.log"
fi
