"""Testing CLI for Stage 4 CRIS-lite."""
...
from .dataset import CRISLiteEvalDataset, build_text_collate_fn
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Test Stage 4 CRIS-lite.")
    parser.add_argument("--pth_path", default="./snapshot/CRIS_lite_baseline/Net_epoch_best.pth")
    parser.add_argument("--eval_root", default="Dataset/Stage4CRISLite_baseline/eval")
    parser.add_argument("--save_dir", default="./pred/CRIS_lite_baseline")
...
def load_model(checkpoint_path: Path, device: torch.device) -> tuple[CRISLiteModel, str]:
    checkpoint = torch.load(checkpoint_path, map_location="cpu")
    model_kwargs = checkpoint.get("model_kwargs", {})
    model = CRISLiteModel(**model_kwargs).to(device)
...
def main() -> None:
    ...
    for dataset_name in dataset_names:
        ...
        dataset = CRISLiteEvalDataset(str(manifest_path), testsize=args.testsize)
        ...
        with torch.no_grad():
            for batch in tqdm(data_loader, desc=f"Stage4 test {dataset_name}", leave=False):
